	...
	const properties = [
		{
			...
			headline: 'Reside on Richmond',
			sold_by_title: 'Development by:',
			sold_by: 'Owner',
			address: '123 Main St',
			city: 'Toronto',
			province: 'ON',
		...
		<>
			...
			>
				<div className={`row ${styles.row}`}>
					...
					>
						{properties.map((property: Property_Interface, index: number) => (
							<PropertyCard headline={property.headline} key={index} />
						))}
					</ul>
				</div>
			</section>
